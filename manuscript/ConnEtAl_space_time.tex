...
\begin{document}
\runningheads{P.~B.~Conn \& Coauthors}{Space-time abundance models}

\papertype{Article}

\title{Using spatio-temporal models to estimate animal abundance and infer ecological dynamics from transect data \footnotemark[2]}

\author{P.~B. ~Conn\affil{1}\corrauth, Coauthors\affil{2}}


\address{\affilnum{1}National Marine Mammal Laboratory, Alaska Fisheries Science Center, NOAA National Marine Fisheries Service, 7600 Sand Point Way NE, Seattle, WA 98115;\affilnum{2}Coauthors address}
...
\small
\begin{enumerate}
\item  Ecologists often estimate animal abundance using models fit to count data from transect surveys. Such models often require that animal density remains constant across the landscape where sampling is being conducted.  This assumption is problematic for animals inhabiting dynamic landscapes or otherwise exhibiting considerable spatio-temporal variation in density, and may be an impediment to those wishing to infer how changes in environmental conditions affect spatial distribution.  A variety of models have been developed for analyzing spatio-temporal variation in count data, but there has been little comparison of the efficacy of alternative modeling approaches for estimating animal abundance.
...
\keywords{Animal abundance, resource selection, process convolution, restricted spatial regression, spatio-temporal model, transect survey}
...
Development of model-based estimation procedures for transect data have largely focused on inclusion of spatial variation in abundance without as much consideration of temporal variation.  Notable exceptions include \citet{VerHoefJansen2007}, who included changing habitat covariates and separable spatial and temporal effects in count models for harbor seals; \citet{MillerEtAl11jwm}, who allowed bird density to rely on habitat covariates that varied between sample occasions \citep[using an N-mixture formulation; see][]{Royle2004}; \citet{Moore2011}, who allowed the modelled density of fin whales to vary among years and spatial strata; and \citet{VerHoefEtAl2014}, who conceptualized phocid seal counts as a function of both changing habitat covariates and spatially autocorrelated random effects.  Hierarchical generalized linear mixed models incorporating both spatial and temporal effects are also prevalent in analysis of population trend from count data \citep[e.g.][]{SauerLink2011,RossEtAl2012}, but it is not often possible to scale up from these models to the estimates of absolute density or abundance that are often needed for population management.
...
\subsection{Separable space-time (SST) formulation}

The first approach we describe is a simple descriptive model for $\mu_{s,t}$ that includes separable spatial and
temporal structure.  To induce this structure, we modify eqn. \ref{eq:glmm} as
\begin{eqnarray*}
  \mu_{s,t} & = & {\bf X}_{s,t} \boldsymbol{\beta} + \eta_s + \gamma_t + \epsilon_{s,t},
\end{eqnarray*}
and specify an AR(1) temporal autocorrelation structure on $\gamma_t$:
\begin{eqnarray*}
   \gamma_t & = & \begin{cases}
                    \sigma_\gamma Z_{\gamma,t} & \text{if }t=1 \\
                    \phi \gamma_{t-1} + \sigma_\gamma Z_{\gamma,t} & \text{if }t>1.
                  \end{cases}
\end{eqnarray*}
Here, $\sigma_\gamma$ gives the standard deviation of time effects, $Z_{\gamma,t}$ denote standard normal random variables, and $\phi$ describes an autoregressive parameter constrained to take on values in (-1,1).  Note that \citet{VerHoefJansen2007} considered a similar structure, but also included zero inflation.  We consider a dimension reduced version for spatial random effects $\eta_s$, using a spectral decomposition of the standard intrinsic conditionally autoregressive (ICAR) model \citep{RueHeld2005}.  We follow a number of recent ecological studies \citep[e.g.][]{HootenEtAl2013,JohnsonEtAl2013,ConnEtAl2014} in choosing a restricted parameterization for spatial random effects that removes any potential confounding with habitat covariate effects while simultaneously achieving dimension reduction \citep[i.e. restricted spatial regression - RSR;][]{ReichEtAl2006,HodgesReich2010,HughesHaran2013}.   This approach, together with details on prior specification and MCMC sampler implementation, are formally described in Appendix S1.
...
\begin{eqnarray*}
  ...
  \alpha_{mt} & = &  \begin{cases}
                  ...
                  \end{cases}
\end{eqnarray*}
So basically a spatial convolution where weights associated with each of the knots evolves in time according to an AR1 process... inspired by \citet{Higdon2002}.  Other suggestions?
...
obtained seal count data for transects flown over the Bering Sea in April and May of 2012 from the
N
