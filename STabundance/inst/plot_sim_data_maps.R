...
load('d:/ST_out/ST_out_gen1_est_CPIF_trans_2_sim52.Rdata')

load("./sim_generic_data/simdata_genRS2closed_trans5_sim52")
Data=Sim.data$Data
...
p4 <- ggplot(subset(DF,Type=="Estimate"))+aes(Easting,Northing,fill=Response)+geom_raster()+facet_grid(Type~Time,scales='free')+tmp.theme+theme(strip.text.x=element_blank(),strip.background=element_rect(fill="white"),plot.margin=unit(c(-1.5,0.5,1,1),"line"))+scale_fill_gradientn(limits=c(0,430),colours=myPalette(100))
