...
Control=list(iter=1100,burnin=100,thin=10,srr.tol=0.5,predict=TRUE,MH.Z=rep(0.01,t.steps),MH.mu=rep(0.2,S),MH.N=0.05,MH.omega=rep(0.05,t.steps),adapt=FALSE,fix.tau.epsilon=FALSE)
model=~0+matern+matern2
Predictors=c("matern","matern2")
Prior.pars=NULL
Area.adjust=NULL

if(Est.mods[iest]=="OPRS")MCMC=mcmc_CPIF(Predictors=Predictors,Data=Data,Control=Control)

#if(Est.mods[iest]=="RS2")MCMC=NULL   



Control=list(iter=5100,burnin=4100,thin=20,predict=FALSE,MH.N=0.01,MH.omega=rep(0.001,t.steps),adapt=TRUE,fix.tau.epsilon=FALSE)        
